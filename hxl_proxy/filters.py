...
def add_rename_filter(source, args, index):
    ...
    oldtag = TagPattern.parse(profile.get('rename-oldtag%02d' % index))
    newtag = TagPattern.parse(profile.get('rename-newtag%02d' % index))
    header = profile.get('rename-header%02d' % index)
